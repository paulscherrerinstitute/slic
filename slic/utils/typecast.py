...
def downcast(obj, cls):
    ...
    obj_cls = type(obj)
    _ensure_subclass(cls, obj_cls)
    return _cast(obj, cls)

def upcast(obj, cls):
    """Upcast (generalize/widen) object obj to parent type cls"""
    obj_cls = type(obj)
    _ensure_subclass(obj_cls, cls)
    return _cast(obj, cls)

def _ensure_subclass(child_cls, parent_cls):
    if not issubclass(child_cls, parent_cls):
        _raise_error(child_cls, parent_cls)

def _raise_error(child_cls, parent_cls):
